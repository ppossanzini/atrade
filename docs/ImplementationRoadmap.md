...
**Questioni aperte prima di scrivere il retrieval.** Risolte il 2026-09-21 (ADR-0026): l'episodio che entra in memoria e un **evento operativo concluso** con contesto misurabile (`OperationalEpisode`), non il trade chiuso, che resta il termine riservato alla Win/Loss History. Meritano memoria le anomalie piu le esecuzioni chiuse nominali, e la scrittura avviene quando l'esito e noto perche la memoria e append-only. La resa in testo e versionata (`OperationalEpisodeRenderer.TextVersion = v1`) ed e resa in cultura invariante: con una locale a virgola decimale il testo, e quindi il vettore, cambierebbe per una ragione invisibile nel codice. Resta da fare il **cablaggio**: chi raccoglie i fatti per ogni categoria e in quale punto della catena vengono scritti.
