...
**Blocco esterno prima di scrivere il seam di embedding.** `Jigen.SemanticTools` — il progetto che contiene `OnnxEmbeddingGenerator`, `OnnxExecutionProvider` e i profili `Nomic`/`Qwen3`/`SigLip2`/`Custom` — **non è pubblicato**: 404 su nuget.org e sul feed privato, e il suo `.csproj` non ha configurazione di pack (`PackageId`, `Version`, `GeneratePackageOnBuild`). Le sue dipendenze sono invece tutte pubbliche (`Microsoft.ML.OnnxRuntime` 1.27.0, `Microsoft.ML.Tokenizers` 2.0.0, `Microsoft.ML.OnnxRuntime.Extensions` 0.14.0, `OpenCvSharp5`). Serve la pubblicazione, esattamente come è servita per `Jigen.Primitives`: un `ProjectReference` al checkout locale è stato scartato perché renderebbe l'applicazione dipendente da un percorso invece che da una versione. Va inoltre procurato il checkpoint (`model.onnx` più tokenizer): nel repository Jigen non esiste alcun file ONNX di testo, solo un modello di visione usato dai test.
