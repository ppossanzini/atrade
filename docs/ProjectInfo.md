...
## ADR-0031 - The operator workstation uses a light trading-desk palette

- Date: 2026-09-21
- Status: Accepted
- Context: The owner rejected the dark interface. The application is a dense operational workstation, so the replacement must improve daylight legibility without turning semantic trading states into decorative color.
- Decision:
  - The canvas is a cool paper gray, raised panels are white and primary text is graphite.
  - Exchange blue is the only action/navigation accent; green, amber and red remain reserved for operational meaning.
  - Element Plus is themed through its CSS variables. Shared panels use restrained borders and a shallow light-mode shadow; component layouts remain unchanged.
- Consequences: the complete application changes visual direction from the shared token layer, with no per-view repaint and no dark-mode CSS dependency.

## ADR-0032 - Basket composition does not contain a manually selected market class

- Date: 2026-09-21
- Status: Accepted
- Context: The operator identified `Fx`/`Metal`/`Index` as redundant input. The symbol or provider catalog is authoritative, while a manually selected class can contradict the actual instrument.
- Decision:
  - `Market` is removed from the basket leg HTTP contract and from every client view of basket, proposal, risk and execution legs.
  - The simulated source resolves only explicit symbol profiles; it no longer invents data through a market-class fallback.
  - Existing persistence columns remain temporarily for schema compatibility but are no longer operator input or part of the public contract. Their physical removal requires a separately generated EF migration by a human developer.
- Consequences: the operator specifies the instrument once through its symbol, and an unknown simulated symbol fails closed. No migration is generated by this change.
