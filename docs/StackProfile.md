...
| embeddings | In-process ONNX via `Jigen.SemanticTools` **1.3.2**, the same runtime that ranks the vectors; checkpoint `nomic-embed-text-v1.5` at `/data/onnx/nomic-embed-text-v1.5`, variant `model_int8.onnx` (137 MB) with `tokenizer.onnx`. Active and verified end to end | Owner decision 2026-09-20: the structure must work atomically, no external engine (ADR-0025); quantised variant chosen 2026-09-21 on measured evidence | td-backend-dev |
