...
| embeddings | In-process ONNX via `Jigen.SemanticTools` (`OnnxEmbeddingGenerator`), same runtime that ranks the vectors; checkpoint `nomic-embed-text-v1.5` as a file on disk. **Blocked: the package is not published** (nuget.org and private feed both 404, no pack configuration in its csproj) | Owner decision 2026-09-20: the structure must work atomically, no external engine (ADR-0025) | td-backend-dev |
